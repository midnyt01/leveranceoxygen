...
const NotificationPanel = ({notificationToggle}) => {
  ...
  const {sellerNotifications, setSellerNotifications, notificationCount, setNotificationCount} = useContext(SellerNotificationsContext)
  ...
  const findAndRemove = (id) => {
    let newArray = sellerNotifications
    for (let i = 0; i < newArray.length; i++) {
      if (sellerNotifications[i].messageId === id) {
        newArray.splice(i, 1);
        break;
      }
    }
    notificationToggle()
    setSellerNotifications(newArray)
